...
export async function updateOrderItems(req: any, res: Response) {
  try {
    const restaurantId = req.user?.restaurantId;
    if (!restaurantId) {
      return res.status(400).json({ success: false, message: 'Missing restaurantId in token' });
    ...
    if (!Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ success: false, message: 'Danh sách món ăn (items) không hợp lệ' });
    }

    for (const item of items) {
      if (!item.dishId && !item.comboId) {
         return res.status(400).json({ success: false, message: 'Mỗi item phải có dishId hoặc comboId' });
      }
      if (typeof item.quantity !== 'number' || item.quantity <= 0) {
         return res.status(400).json({ success: false, message: 'Số lượng (quantity) phải lớn hơn 0' });
      }
    }

    const updated = await orderService.updateOrderItems(restaurantId, orderId, items);
    ...
    console.error('[OrderController] updateOrderItems error:', error);
    const statusCode = error.statusCode || 500;
    return res.status(statusCode).json({
      success: false,
      message: error.message || 'Lỗi hệ thống khi cập nhật danh sách món ăn',
    });
  }
}
